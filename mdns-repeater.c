...
static void show_help(const char *progname) {
	...
	fprintf(stderr, "UCIfication by Majenko Technologies, 2016\n\n");
...
int uci_get_option(struct uci_context *ctx, char *tuple, char *buf) {
    struct uci_ptr ptr;
...
int main(int argc, char *argv[]) {
    ...
    char *ifaces[20]; // should be enough?!
    int ifnum = 0;
    ...
    if (uci_lookup_ptr(ctx, &ptr, "network", true) != UCI_OK) {
        fprintf(stderr, "Error getting UCI network configuration\n");
		uci_free_context(ctx);
        exit(2);
    }

    if (!(ptr.flags & UCI_LOOKUP_COMPLETE)) {
        fprintf(stderr, "UCI lookup failed\n");
        uci_free_context(ctx);
        exit(2);
    }
    ...
    uci_foreach_element( &pkg->sections, e) {
        struct uci_ptr entptr;
        struct uci_ptr typeptr;

        struct uci_section *s = uci_to_section(e);

        if (!strcmp(s->type, "interface")) {
            snprintf(tuple, 100, "network.%s.repeat", s->e.name);
            if (uci_get_option(ctx, tuple, val) <= 0) {
                continue;
            }
            ...
            snprintf(tuple, 100, "network.%s.type", s->e.name);
            if (uci_get_option(ctx, tuple, val) > 0) {
                if (strcmp(val, "bridge") == 0) {
                    isBridge = 1;
                }
            }
                
            if (isBridge) {
                ifaces[ifnum] = malloc(strlen(s->e.name) + 4);
                sprintf(ifaces[ifnum], "br-%s", s->e.name);
                ifnum++;
                continue;
            } 

            snprintf(tuple, 100, "network.%s.ifname", s->e.name);
            if (uci_get_option(ctx, tuple, val) > 0) {
                ifaces[ifnum] = strdup(val);
                ifnum++;
                continue;
            }

            snprintf(tuple, 100, "network.%s._orig_ifname", s->e.name);
            if (uci_get_option(ctx, tuple, val) > 0) {
                ifaces[ifnum] = strdup(val);
                ifnum++;
                continue;
	...
	int i;
	for (i = 0; i < ifnum; i++) {
		int sockfd = create_send_sock(server_sockfd, ifaces[i], &socks[num_socks]);
		if (sockfd < 0) {
			log_message(LOG_ERR, "unable to create socket for interface %s", ifaces[i]);
			r = 1;
			goto end_main;
		}
		num_socks++;
